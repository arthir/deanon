...
def read_clicks(url_count):
    clicks_by_user = dict()     # read clicks [url] -> users
    clicked_tuples = dict()     # user -> [url1, url2,...]
    ...
    with open(click_file, "rb") as f:
        for line in f:
            ...
            if url not in url_translation:
                ...
                url_count += 1
            url_number =  url_translation[url]
            if url_number not in clicks_by_user:
                clicks_by_user[url_number] = set()
            clicks_by_user[url_number].add(user)
            #real_ids[user] = real_user
            if user not in clicked_tuples:
                clicked_tuples[user] = set()
            clicked_tuples[user].add(url)
    #print "done reading clicks"
    return [clicks_by_user,clicked_tuples,url_count]


[clicks_by_user,clicked_tuples,url_count] = read_clicks(url_count)
...
for url in urls_ordered:
    ...
    for url_tuple in potential_ids_by_url.keys():
        ...
        if len(receivers_combined) < len(potential_ids_by_url[url_tuple]):  # this new url actually adds info
            if new_tuple in clicked_tuples.values(): # this tuple is actually a set of things that have been clicked on
